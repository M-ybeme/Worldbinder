...
**Phase 11 — Monitoring: Sentry SDK wired in, env-gated** [Done — see 0.14.11]
...
- **Resolution — `apps/api` uses `@sentry/nestjs` instead of raw `@sentry/node`** (it wraps `@sentry/node` with NestJS-appropriate defaults and ships its own `SentryModule`/`SentryGlobalFilter`, real exports confirmed by reading the installed package's own type declarations rather than assumed from memory). A new `apps/api/src/instrument.ts` / `apps/worker/src/instrument.ts` — each the very first import in their respective `main.ts`, since Sentry's Node SDK needs to patch modules via OpenTelemetry before anything else `require`s them — calls `Sentry.init({ dsn, environment })` only `if (env.SENTRY_DSN)`; unset means `init` is never called at all, not called with an empty DSN. `AppModule` imports `SentryModule.forRoot()` first and registers `SentryGlobalFilter` as `APP_FILTER` (not `app.useGlobalFilters()`, so it also applies under Nest's testing module) — it forwards unexpected errors to Sentry and calls through to the base Nest exception behavior either way, a safe no-op with no DSN.
- `apps/worker` had no centralized job-failure capture point at all before this (confirmed by audit: no `.on('failed', ...)` anywhere, `main()`'s own `.catch()` only covers startup). Added `reportJobFailures()` in `main.ts`, attached to all three `Worker` instances (attachment/export/import), logging via the existing `pino` logger and calling `Sentry.captureException` (safe no-op with no DSN).
- `apps/web` had no error boundary anywhere (confirmed by repo-wide grep). `main.tsx` now wraps the router in `Sentry.ErrorBoundary`, with the fallback rendered via the existing `packages/ui` `ErrorState` component (Milestone 13) rather than new UI — `onRetry` calls the boundary's own `resetError`. `Sentry.init` is similarly gated on `VITE_SENTRY_DSN` being set. Production bundle grew from 523.41 kB/157.14 kB gzip (Phase 7's number) to 539.91 kB/162.94 kB gzip — a small, expected cost of the browser SDK.
- **Health check extension**: added `StorageHealthIndicator` (a `HeadBucketCommand` against the configured bucket, the same call `StorageService.onModuleInit()` already made for auto-bucket-creation, now exposed as `StorageService.isHealthy()`) and `QueueHealthIndicator` (BullMQ's `Queue.waitUntilReady()` on `ExportQueueService`, standing in for "the job queue" generally — all three queue services share the same Redis instance and BullMQ version, so they fail/recover together in essentially every real scenario, making one check representative without three redundant connections). Both wrapped in a shared `rejectAfter()` 2-second timeout (`apps/api/src/common/timeout.util.ts`) so a genuinely unreachable dependency reports "down" promptly instead of hanging on the SDK's own much longer retry defaults.
- **Verified the down-path for real, not just the up-path**: with the full stack running locally, stopped the MinIO container mid-session — `GET /health` correctly reported `storage: down`, overall `status: error`, and HTTP 503 within the 2-second timeout; restarting MinIO recovered it on the next request. Confirms the new indicators actually detect a real outage, not just a mocked one.
- **Found and fixed a real bug while wiring this up**: `SENTRY_DSN: z.string().url().optional()` rejects the literal empty string as an invalid URL — but `.env.example`'s own new `SENTRY_DSN=` line is exactly that (dotenv parses `KEY=` as `''`, not an omitted key), so anyone who copied the example file verbatim would have had `pnpm dev`/every test immediately fail on env validation. Added an `optionalUrl()` helper (`packages/config/src/env.ts`, same class of footgun as the existing `booleanString()` helper's own doc comment) that treats an empty string the same as unset, with 4 new regression tests.
- Full suite re-run clean: 19 `packages/config` unit tests (15 + 4 new), 96 API unit tests (92 + 4 new health-indicator specs), 189 API integration tests (including an extended `Health (e2e)` spec asserting `storage`/`queue` are both up against real MinIO/Redis), 31 worker unit tests, 4 worker integration tests, typecheck/lint clean across all four packages/apps.

**Phase 12 — Backup restore drill and migration rehearsal (local)** [Done — see 0.14.12]
...
- **Resolution**: `infrastructure/scripts/backup.sh`/`restore.sh`/`restore-drill.sh` — genuinely environment-agnostic (read `DATABASE_URL` from env, never a hardcoded target) by running `pg_dump`/`pg_restore`/`psql` inside a throwaway `postgres:17-alpine` container (matching the compose Postgres version) rather than requiring client tools on the host, so Docker is the only real dependency. `infrastructure/scripts/rewrite-url.js` uses Node's `URL` class (not sed/regex, which broke on query strings and embedded credentials during design) to rewrite a `localhost`/`127.0.0.1` host to `host.docker.internal` for that throwaway container's benefit only — any other host (a real deployed URL) passes through unchanged, which is exactly what makes the same script work unmodified against a real target later.
- `restore-drill.sh` never touches the real database destructively: backs it up, restores into an isolated `worldbinder_restore_drill` scratch database, verifies row counts match across 8 representative tables, then drops the scratch database and deletes the dump file.
- **Object storage's own backup gap, named directly in this phase's own audit text, is also closed**: `apps/api/src/backup/storage-mirror.ts` (`pnpm --filter @worldbinder/api backup:storage`) mirrors every object in the configured bucket to a local directory, using the same `createS3Client` helper `StorageService` already uses — MinIO locally, R2/S3 later, no code change either way. A database-only backup can't recover uploaded attachments or export archives, since those live in object storage, not Postgres.
- **Migration rollback is now explicitly documented** (`docs/runbooks/backup-restore.md`) as "restore the pre-migration backup" — the only rollback path that exists, since drizzle-kit generates no down-migrations. Also documents `restore.sh`'s destructive `--clean --if-exists` behavior, the object-storage recovery procedure (re-upload mirrored files under their original keys), and what's deferred to Milestone 16 (a live drill against real hosted Postgres, automated/scheduled backups, live alert testing).
- **Verified for real, not just described**: ran `restore-drill.sh` against the actual local dev database (191 users, 120 campaigns, 10,139 entities, 50,020 relationships, 259 sessions, 2,014 plot threads, 502 timeline events, 21 maps) — every table's row count matched after restore, scratch database and dump file cleaned up automatically. Ran `backup:storage` against the real local MinIO bucket — 676 objects (8.6MB) mirrored, file count on disk confirmed to match exactly.
- **Drizzle's transaction-rollback guarantee verified with a real forced-failure test**, not just asserted: `apps/api/test/transaction-atomicity.e2e-spec.ts` runs a `db.transaction(async (tx) => {...})` that inserts a new campaign row, then deliberately violates `campaignMembers`' real `(campaignId, userId)` unique constraint — a genuine Postgres constraint violation, not a synthetic `throw` — and confirms the campaign insert from earlier in that same transaction did not persist. Proves Postgres's own rollback semantics are what's being exercised through Drizzle's transaction wrapper.
- Full suite re-run clean: 190 API integration tests (189 + 1 new), typecheck/lint.

**Phase 13 — Incident runbooks** [Done — see 0.15.0]
...
- **Resolution**: `docs/runbooks/incident-triage.md` — first-response steps (`/health`, structured `pino` logs, `docker ps`) then a section per subsystem (database, Redis/rate-limiting, object storage, background jobs, email delivery, elevated error rates), each grounded in this milestone's own work: the exact `/health` JSON shape and 2-second-timeout behavior (Phase 11), the real `ratelimit:*`/`bull:*` Redis key patterns, the real `attachment_status`/`campaign_export_status`/`campaign_import_status` enum transitions for diagnosing a stuck job, and pointers to the Phase 8 load-test harnesses for confirming a suspected performance regression is real before escalating it. Ends with an explicit "Once Railway/Sentry exist (Milestone 16 — not yet)" placeholder section, per this phase's own scope note.
- `docs/runbooks/security-incident.md` — the `security_events`/`campaign_audit_events` two-audit-trail distinction (which table to check for which kind of event, with their real `type` enum values), a step-by-step for the one security event this app actively defends against (`refresh_reuse_detected`, ADR-0007's rotation-on-use family revocation), repeated-login-failure triage, and a credentials-leaked procedure — including the real, verified distinction that `changePassword` revokes _all_ of a user's sessions while `logout`/`DELETE /auth/sessions/:sessionId` only revoke one (found while writing this: the runbook's first draft conflated the two).
- **Every command and SQL query in both documents was run against the real local stack before being written down**, not composed from memory and left unverified: `GET /health`'s exact response shape, both `security_events` queries, the `campaign_exports`/`campaign_imports`/`attachments` status queries, and both `redis-cli KEYS` patterns (`ratelimit:*`, `bull:campaign-export:*`) — all confirmed against the real dev database/Redis/API, including catching and simplifying an unnecessarily convoluted JSON-pretty-print one-liner in an early draft.
...
**Status against exit criteria (2026-07-16)**: all 13 phases above are done (0.14.1–0.14.12, then 0.15.0 for this final phase). No critical security findings or known authorization leaks were found beyond what each phase's own audit already fixed (documented per-phase above, not re-litigated here). Performance budgets pass against the Phase 8 load-test harnesses at the roadmap's own seeded scale. Backup restore succeeds — verified for real against the actual local dev database and MinIO bucket (Phase 12), not just scripted and assumed. The rollback procedure is documented (`docs/runbooks/backup-restore.md`) and tested (the same Phase 12 drill, plus a real forced-failure test for in-flight transaction rollback). As with Milestone 13's own honest caveat: this is "hardened against the issues each phase's audit actually found," not an external penetration test or a live-fire drill against real hosted infrastructure — Railway/R2/Sentry/a real email provider remain unprovisioned by deliberate, explicit user decision, deferred to Milestone 16.
...
**Beta testing decision (2026-07-16)**: sessions will be in-person and moderated — the user personally observes participants and files issues themselves afterward. This collapses most of this milestone's original deliverable list to "not needed": no in-app feedback-capture UI, no in-app error-report UI, no support-contact channel/UI, no live Sentry project (stays exactly as Milestone 14 left it — env-driven, inert with no `SENTRY_DSN` — real provisioning is Milestone 16's job alongside Railway). "Closed beta users" and "analytics decision" are the user's own logistics, not engineering work. What's left as real work: the demo campaign (§28's exact spec), a Privacy Policy/Terms of Use draft, and beta release notes for participants.

**Phase 1 — Demo-content script scaffolding** [Done — see 0.15.1]

- `apps/api/src/database/seed-perf.ts` is the only precedent for a large seed script, but it does raw Drizzle inserts and explicitly skips `WikiLinksService.refreshLinks()`/`CampaignAuditService.record()` — fine for its own purpose (a performance-testing fixture) but means wiki-links/backlinks don't actually work on that data. Since §28's whole point is demonstrating that connected campaign knowledge is valuable, the demo must go through the real service layer — real HTTP calls against a running dev server, mirroring the login/auth pattern already proven in `apps/api/src/load-test/http-load-test.ts`, not raw inserts.
- New `apps/api/src/demo-content/` directory: a Mailpit-polling helper (small local copy of `test-users.ts`'s `findEmailToken()` pattern, kept independent of the test suite), a tiny TipTap doc builder for real wiki-link mention content, and an orchestrator script wired up through account registration/verification and campaign+membership creation only (no campaign content yet) — verified end to end against a running dev stack before any content gets written. New `apps/api` package.json script `seed:demo`, matching the `db:seed`/`db:seed:perf`/`load-test:http` naming convention.

**Phase 2 — Demo campaign content build** [Done — see 0.15.1]

- The full §28-spec dataset for a proposed setting ("Ashgate Crossing" — a low-fantasy political/horror campaign, a river-crossing town in succession crisis while a drowned-god cult uses the chaos as cover underneath its own chapel): 25–40 entities across all 11 `entityTypeEnum` values, 40–60 relationships (built-in types plus 1–2 custom ones), 6–10 plot threads, 4–6 sessions, 10–15 timeline events, 2 maps, 6 real attachment uploads spanning every attach point (map backgrounds, entity portrait, plot-thread handout, session image, campaign cover).
- Two real constraints found by reading the actual controllers, not assumed: attachments/map images only become usable once `status === 'ready'`, set asynchronously by `apps/worker`'s BullMQ job — the script must poll for readiness before linking an image, with the worker actually running; and the request budget (low-to-mid 200s) sits comfortably under the Milestone 14 global rate limit (300 req/60s/IP) but the script should still be paced rather than assumed risk-free.
- Written and run incrementally — entities verified in the real web UI before relationships that depend on them get written, etc. — not as one untested script.

**Phase 3 — Interconnection and revision pass** [Done — see 0.15.1]

- `RevisionRecorderService` merges same-actor edits made within the same ~30-minute window into a single revision row — §28's "multiple revisions" needs a second real actor (the editor-role demo account) doing a later edit pass, not a sleep. This pass enriches entity content with real wiki-link mentions and makes narrative-driven updates (e.g. the dying lord's death) as that second actor, producing genuine second revisions.
- One deliberate bad-edit-then-restore beat on a minor entity, since `RevisionsService.restore()` always forces a new revision regardless of timing — exercises the restore path for real.

**Phase 4 — Verification pass** [Done — see 0.15.1]

- Scripted read-back: as the GM account, run the designed search examples and confirm cross-referenced entities/threads/locations surface together; as the player account, confirm `gm_only` content is filtered/403s and doesn't leak into that account's own search results — a real end-to-end permission check on real data, directly serving the beta goal "find permission edge cases." Caught two real bugs before they shipped: a flawed test assumption around already-revealed content, and a genuine content-security leak (a public-content field named a secret NPC that should only have surfaced via the intended in-fiction reveal).
- One manual walkthrough in the actual browser (not just API responses): wiki-links render as clickable mentions, backlinks populate on entity pages, both map layers display with correct visibility, revision history shows the real multi-actor and restore entries.
- Full existing test suite re-run clean. One false alarm during this pass: `pnpm test:integration` run in full showed 4 failing suites (attachments/maps/exports/imports e2e-specs) — root cause traced to this session's own long-running `pnpm dev` API/worker processes still being up, silently consuming the same real Redis/BullMQ attachment-processing queue as the test suite and racing against tests' own state setup. Not an application bug; stopping the dev stack before running the integration suite made it pass clean (190/190). Worth remembering: the integration suite assumes no other consumer is competing for the same queue.

**Phase 5 — Privacy Policy and Terms of Use** [Done — see 0.15.2]
...
**Phase 2 — Missing and partial ADRs** [Done — see 0.16.2]
