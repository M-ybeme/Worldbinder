...
**Resolved** (Milestone 14 Phase 4b/Phase 9 — this section originally listed these as open gaps found by Phase 1's audit; corrected 2026-08-12 after finding the fix had shipped but this note was never updated): the refresh cookie's `Secure` flag was checked and found already environment-conditional (`secure: NODE_ENV === 'production'`), no fix needed. `JWT_ACCESS_SECRET`'s placeholder-passes-validation gap and `STORAGE_ACCESS_KEY_ID`/`STORAGE_SECRET_ACCESS_KEY`'s silent dev-credential fallback were both closed by a `superRefine` guard (`rejectDevOnlyValuesOutsideDevAndTest`, `packages/config/src/env.ts`) rejecting known local-dev-only values outside `development`/`test` — applied to `apiEnvSchema` in Phase 4b, then extended to `workerEnvSchema` in Phase 9 after an audit found the worker had the same storage-credential gap the API-only fix had missed.
...
Per an explicit product decision (2026-07-15): the application is to be feature-complete before any real hosting environment is provisioned. This milestone makes every hosting-dependent integration point (CORS, CSP, cookies, secrets, storage backend, email transport, monitoring, health checks, backup/restore tooling) environment-driven and swappable, but does **not** create a Railway project, R2 bucket, Resend account, or Sentry project, and does not run any live drill against real hosted infrastructure. That provisioning work, plus the live backup restore drill, live alert testing, and production smoke tests, is explicitly deferred to Milestone 16 (v1.0 Release Candidate) — see that milestone's own forward-reference note in the roadmap. (The provider choice among Resend/Postmark/SES was still open when this milestone shipped; it was finalized to Resend on 2026-08-12, before any of it was actually provisioned — see Milestone 16's decision update.)
